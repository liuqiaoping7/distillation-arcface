...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck,data):
...
def resnet_student(units, num_stages, filter_list, num_classes, bottle_neck,data):
    bn_mom = config.bn_mom
    workspace = config.workspace
    kwargs = {'version_se' : config.net_se,
        'version_input': config.net_input,
    ...
    if version_input==0:
      #data = mx.sym.BatchNorm(data=data, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='bn_data')
      data = mx.sym.identity(data=data, name='student_id')
      data = data-127.5
      data = data*0.0078125
      body = Conv(data=data, num_filter=filter_list[0], kernel=(7, 7), stride=(2,2), pad=(3, 3),
                                no_bias=True, name="student_conv0", workspace=workspace)
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='student_bn0')
      body = Act(data=body, act_type=act_type, name='student_relu0')
      #body = mx.sym.Pooling(data=body, kernel=(3, 3), stride=(2,2), pad=(1,1), pool_type='max')
    elif version_input==2:
      data = mx.sym.BatchNorm(data=data, fix_gamma=True, eps=2e-5, momentum=bn_mom, name='student_bn_data')
      body = Conv(data=data, num_filter=filter_list[0], kernel=(3,3), stride=(1,1), pad=(1,1),
                                no_bias=True, name="student_conv0", workspace=workspace)
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='student_bn0')
      body = Act(data=body, act_type=act_type, name='student_relu0')
    ...
    for i in range(num_stages):
      ...
      #    name='stage%d_unit%d' % (i + 1, 1), bottle_neck=bottle_neck, **kwargs)
      body = residual_unit(body, filter_list[i+1], (2, 2), False,
        name='student_stage%d_unit%d' % (i + 1, 1), bottle_neck=bottle_neck, **kwargs)
      for j in range(units[i]-1):
        body = residual_unit(body, filter_list[i+1], (1,1), True, name='student_stage%d_unit%d' % (i+1, j+2),
          bottle_neck=bottle_neck, **kwargs)

    if bottle_neck:
      body = Conv(data=body, num_filter=512, kernel=(1,1), stride=(1,1), pad=(0,0),
                                no_bias=True, name="student_convd", workspace=workspace)
      body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='student_bnd')
      body = Act(data=body, act_type=act_type, name='student_relud')

    fc1 = symbol_utils.get_fc1_student(body, num_classes, fc_type)
    return fc1

def get_symbol(data,is_student):
    """
    ...
    if(is_student):
        net = resnet_student(units       = units,
