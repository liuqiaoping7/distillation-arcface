...
'''test images root'''
card_root = u'data/faceGate_795_id_clean_112_112/card_distractor/card'
catch_root = u'data/faceGate_795_id_clean_112_112/catch'
distractor_root = u'data/faceGate_795_id_clean_112_112/card_distractor/distractor'
...
def get_symbol(args):
  ...
  config.net_name = 'fresnet_seprate'
  config.num_layers = 100
  embedding_teacher = eval(config.net_name).get_symbol(data=data,is_student=False)
  # embedding_teacher = mx.sym.stop_gradient(embedding_teacher)
  embedding_teacher = mx.symbol.BlockGrad(embedding_teacher)
  # ter = mx.sym.split_v2(embedding_teacher, axis=0,indices_or_sections=2)
  # ter0 = mx.sym.L2Normalization(ter[0], mode='instance')
  # ter1 = mx.sym.L2Normalization(ter[1], mode='instance')
  # tercoss = mx.sym.elemwise_mul(ter0,ter1)
  nembedding_teacher = mx.symbol.L2Normalization(embedding_teacher, mode='instance')
  tercoss = mx.sym.dot(nembedding_teacher,nembedding_teacher,transpose_b=True)
  config.num_layers = 34
  embedding_student = eval(config.net_name).get_symbol(data=data,is_student=True)
  # stu = mx.sym.split_v2(embedding_student, axis=0,indices_or_sections=2)
  # stu0 = mx.sym.L2Normalization(stu[0], mode='instance')
  # stu1 = mx.sym.L2Normalization(stu[1], mode='instance')
  # stucoss = mx.sym.elemwise_mul(stu0,stu1)
  nembedding_student = mx.symbol.L2Normalization(embedding_student, mode='instance')
  stucoss = mx.sym.dot(nembedding_student,nembedding_student,transpose_b=True)
  cos_diff = mx.sym.elemwise_sub(tercoss,stucoss)
  cos_abs = mx.sym.abs(cos_diff)
  cos_avg = mx.sym.sum(cos_abs)/args.per_batch_size/args.per_batch_size
  cos_loss = mx.sym.MakeLoss(cos_avg)

  config.net_name = 'fresnet'
  all_label = mx.symbol.Variable('softmax_label')
  gt_label = all_label
  is_softmax = True
  embedding = mx.symbol.BlockGrad(embedding_student)
...
def train_net(args):
    ...
    prefix = os.path.join(args.models_root, '%s-%s-%s'%(args.network, args.loss, args.dataset), 'model')  # save param prefix
    prefix_dir = os.path.dirname(prefix)
    print('prefix', prefix)
    if not os.path.exists(prefix_dir):
      os.makedirs(prefix_dir)    # creative save subdir
    ...
    path_imgrec = os.path.join(data_dir, "train.rec")   # training record data path
    ...
    begin_epoch = 0
    fixed_param_names = []
    if len(args.pretrained)==0:    # if not have pretrained model
      arg_params = None
      aux_params = None
      sym = get_symbol(args)
      ...
      ter_sym, ter_arg_params, ter_aux_params = mx.model.load_checkpoint('./teacher_model/card', 35)
      fixed_param_names.extend(list(ter_sym.get_internals().list_arguments()))
      arg_params.update(ter_arg_params)
      aux_params.update(ter_aux_params)
      sym = get_symbol(args)
    ...
    def ver_test(nbatch):
      ...
      for i in range(len(ver_list)):
        ...
        results.append(acc2)
      '''start catch testing'''
      eval_1vsN.catch_test(model, card_root, catch_root, distractor_root,config.batch_size)
    ...
    def _batch_callback(param):    # param include: epoch, nbatch, eval_metric, locals
      #global global_step
      global_step[0]+=1
    ...
    #print(train_dataiter.provide_data.shape)
    '''for test initialization before training'''
    #'''
    # model.bind(data_shapes=train_dataiter.provide_data,
    #            label_shapes=train_dataiter.provide_label)
    # model.set_params(arg_params, aux_params, allow_missing=True)
    # res_leave = ver_test(nbatch=0)
    #'''
